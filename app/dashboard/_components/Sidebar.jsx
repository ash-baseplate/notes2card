  ...
  return (
    ...
    <div className='h-screen shadow-md p-5'>
      ...
      <div className='mt-10'>
        ...
        <div className='mt-5'>
          {MenuList.map((menu, index) => (
            <div key={index} className={`flex items-center gap-4 p-3 rounded-md cursor-pointer hover:bg-gray-200 ${path === menu.path && 'bg-gray-200 font-semibold'}`} >
              <menu.icon />
              <span>{menu.name}</span>
            </div>
          ))}
      ...
      <div className='border p-2 bg-slate-100 rounded-lg absolute bottom-10 w-[85%] '>
        ...
        <Link href={"/dashboard/upgrade"} className='text-primary text-xs mt-2'>Upgrade to Create More</Link>
      </div>
